...
public class ApiGatewayHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(
        ...
        try {
            ...
            context.getLogger().log("ApiGateway error: " + ex.getMessage());
            Map<String, Object> responseMap = ResponseUtil.errorResponse(
    ...
    private ApiGatewayRequestDto extractRequest(APIGatewayProxyRequestEvent event) {
        ...
        if (headers == null || headers.get("action") == null) {
            throw new BusinessException(
                    ErrorMessage.INVALID_REQUEST, "Headers missing or invalid.");
        }
        ...
        try {
            ...
            if (action == Actions.REGISTER || action == Actions.CANCEL) {
                if (platformStr == null || platformStr.isBlank()) {
                    throw new BusinessException(ErrorMessage.INVALID_REQUEST, "Platform is required for REGISTER and CANCEL actions");
                }
            ...
            RegisterHeaderDto header =
                    new RegisterHeaderDto(
                            transactionId,
                            Services.fromValue(serviceStr),
                            platform,
                            action);
    ...
    private void handleRegister(ApiGatewayRequestDto request) {
        RegisterUserDto body =
                mapper.convertValue(request.body(), RegisterUserDto.class);
    ...
    private void handleCancel(ApiGatewayRequestDto request) {
        DeleteTokenDto body =
                mapper.convertValue(request.body(), DeleteTokenDto.class);
        String transactionId = request.header().transactionId();
        Services service = request.header().service();
        Platform platform = request.header().platform();
        String deviceToken = body.deviceToken();
        ...
        try {
            ...
            DeviceTokenEntity tokenEntity = deviceTokenService.findTokenByDeviceTokenAndUserId(deviceToken, userId);
            if (tokenEntity == null) {
    ...
    private void handleSend(ApiGatewayRequestDto request) {
        SendPushDto body =
                mapper.convertValue(request.body(), SendPushDto.class);
    ...
    private void handleSendAll(ApiGatewayRequestDto request) {
        SendAllPushDto body =
                mapper.convertValue(request.body(), SendAllPushDto.class);
    ...
    private Map<String, Object> parseBody(APIGatewayProxyRequestEvent event) {
        if (event.getBody() == null) {
            throw new BusinessException(
                    ErrorMessage.INVALID_REQUEST, "Request body is missing.");
        }
        ...
        try {
            ...
            throw new BusinessException(
                    ErrorMessage.INVALID_REQUEST, "Failed to parse request body.");
        }
    }
    ...
    private APIGatewayProxyResponseEvent convertToApiGatewayResponse(Map<String, Object> responseMap) {
